...
function ViewBlogs() {
  ...
  const handleEditToggle = (blogId) => {
    setBlogs(prevBlogs =>
      prevBlogs.map(blog =>
        blog.id === blogId ? { ...blog, isEditing: !blog.isEditing } : blog
      )
    );
  };
  ...
  // Save edited blog (date + description + image)
  const handleSave = async (blogId) => {
    const blogToSave = blogs.find(b => b.id === blogId);
    if (!blogToSave) return;

    const updatedBlog = {
      date: blogToSave.date || "",
      description: blogToSave.description || "",
      image: blogToSave.image || "",
    };
    ...
    try {
      ...
      setBlogs(prevBlogs =>
        prevBlogs.map(blog =>
          blog.id === blogId ? { ...blog, ...updatedBlog, isEditing: false } : blog
        )
      );
  ...
  return (
    <div className='md:ml-64 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12'>
      <h1 className='text-2xl font-bold text-gray-900 px-3 border-l-4 border-brandyellow'>
        Our Blogs
      </h1>
        ...
        {blogs.map((blog) => (
          <div key={blog.id} className="bg-white rounded-xl shadow-md overflow-hidden h-full flex flex-col">
            ...
            <div className="relative h-48 md:h-96 w-72">
              <img src={blog.image} alt="Blog" className="w-full h-full object-cover rounded-2xl" />
              {blog.isEditing && (
                  ...
                  <div className="mb-4">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setBlogs(prevBlogs =>
                          prevBlogs.map(b =>
                            b.id === blog.id ? { ...b, date: e.target.value } : b
                          )
                        )
                      }
                    ...
                    <div className="border border-gray-300 rounded-md h-48 overflow-hidden">
                      <ReactQuill
                        ...
                        onChange={(value) =>
                          setBlogs(prevBlogs =>
                            prevBlogs.map(b =>
                              b.id === blog.id ? { ...b, description: value } : b
                            )
                          )
                        }
                        className="h-40"
